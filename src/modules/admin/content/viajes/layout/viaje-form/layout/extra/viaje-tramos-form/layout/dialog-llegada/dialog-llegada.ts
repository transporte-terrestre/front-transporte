...
export class DialogLlegadaComponent implements OnInit, OnDestroy {
  ...
  private validateForm(): boolean {
    ...
    const requiredFields = [
      { control: 'nombreLugar', label: 'Lugar' },
      { control: 'latitud', label: 'Ubicación del mapa' },
      { control: 'longitud', label: 'Ubicación del mapa' },
      { control: 'fecha', label: 'Fecha de llegada' },
      { control: 'hora', label: 'Hora de llegada' },
      { control: 'kilometrajeActual', label: 'Kilometraje final' },
    ];
    ...
    if (this.form.get('kilometrajeActual')?.hasError('min')) {
      this.toastService.warning('El kilometraje final no puede ser negativo.');
      return false;
    }

    if (this.form.invalid) {
      this.toastService.warning('Revisa los datos ingresados antes de registrar la llegada.');
      return false;
    }
