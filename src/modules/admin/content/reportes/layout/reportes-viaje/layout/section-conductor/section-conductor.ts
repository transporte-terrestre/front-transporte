...
@Component({
  selector: 'app-section-conductor',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './section-conductor.html',
  styleUrl: './section-conductor.css',
})
export class SectionConductor implements OnInit {
  ...
  generarReporte() {
    ...
    const id = this.selectedConductorId() || 0;

    if (id === 0 && !this.selectedEntityName()) {
      this.selectedEntityName.set('Todos los Conductores');
    }
